function [S1,c,S0,S0fdint,S0bdint] = iterateSearchMap(fdt,bdt,S0,c,varargin)
...
[S1fd,~,c] = volumeMap(fdt,S0,c,varargin{:});

if isTalkative
    disp('--------------------------------')
   disp('Beginning backward volumeMap...') 
end
[S1bd,S0,c] = volumeMap(bdt,S0,c,varargin{:});
...
% 
% 
flatS1fd = reshape(S1fd,[size(S1fd,1) size(S1fd,2)*size(S1fd,3)]);
uniqueS1fd = unique(flatS1fd.','rows').';

S0fdint = findHitSimplices(S0,uniqueS1fd);

flatS1bd = reshape(S1bd,[size(S1bd,1) size(S1bd,2)*size(S1bd,3)]);
uniqueS1bd = unique(flatS1bd.','rows').';

S0bdint = findHitSimplices(S0,uniqueS1bd);
...
for i = 1:size(S0fdint,3)
    for j = i:size(S0bdint,3)
        S0fdint(:,:,i)
        S0bdint(:,:,j)
        if isequal(S0fdint(:,:,i),S0bdint(:,:,j))
            S1 = cat(3,S1,S0fdint(:,:,i));
        end
    end
end
